...
// run this and see the terminal for any instruction
...
void Homography_matrix()
{
  ...
  for(int m=0;m<4;m++)
  {  
    a[m][0]=lCorners_from[m].x;
    a[m][1]=lCorners_from[m].y;
    a[m][2]=1;
    for(int b=0;b<3;b++)
         a[m][3+b]=0;
    a[m][6]=-(lCorners_from[m].x*lCorners_to[m].x);
    a[m][7]=-(lCorners_from[m].y*lCorners_to[m].x);
  ...
  for(int n=4;n<8;n++)
  {
    for(int c=0;c<3;c++)
          a[n][c]=0;
    a[n][3]=lCorners_from[n-4].x;
    a[n][4]=lCorners_from[n-4].y;
    a[n][5]=1;
    a[n][6]=-(lCorners_from[n-4].x*lCorners_to[n-4].y);
    a[n][7]=-(lCorners_from[n-4].y*lCorners_to[n-4].y);
  } 
  
  for(int d=0;d<4;d++)
     {
       a[d][8]=lCorners_to[d].x;
     }
   
    
  for(int d=0;d<4;d++)
     {
       a[d+3][8]=lCorners_to[d].y;
     } 
      ...
      for(p=0;p<img1.rows;p++)
         {
           for(q=0;q<img1.cols;q++)
              {
                 X=(((a*p)+(b*q)+c)/((g*p)+(h*q)+i));
                 Y=(((d*p)+(e*q)+f)/((g*p)+(h*q)+i));
